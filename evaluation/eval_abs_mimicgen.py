...
from evaluation.gr00tn15_inference import _quat2axisangle
from gr00t.model.policy import Gr00tPolicy
from gr00t.experiment.data_config import EquiMimicgenConfig
...
def _make_env(env_name: str, resolution: int, robosuite_assets_path: str = "", robot: str = "Panda"):
    ...
    # Create with delta=True so we can send zero dummy actions during the wait phase.
    # We switch to absolute mode (use_delta=False) after the wait phase.
...
class Gr00tn15AbsInference:
    """GR00T N1.5 inference wrapper for absolute-action MimicGen evaluation."""
    ...
    def _create_policy(self):
        try:
            data_config = EquiMimicgenConfig()
            policy = Gr00tPolicy(
            ...
            print(f"Number of parameters: {sum(p.numel() for p in policy.model.parameters())}")
            print(f"Loaded abs policy from: {self.model_dir}")
            return policy
        except Exception as e:
            print(f"Failed to load policy: {e}")
            sys.exit(1)

    def get_action(self, obs: dict, task_description: str) -> np.ndarray:
    ...
    def _process_observation(self, obs: dict, task_description: str) -> dict:
        xyz = obs["robot0_eef_pos"]
        rpy = _quat2axisangle(obs["robot0_eef_quat"])
        gripper = obs["robot0_gripper_qpos"]

        img = obs["agentview_image"][::-1].astype(np.uint8)
        wrist_img = obs["robot0_eye_in_hand_image"][::-1].astype(np.uint8)
    ...
    def _convert_action_chunk(self, action_chunk: dict) -> np.ndarray:
        ...
        for t in range(self.infer_chunk):
            ...
            for key in self.action_keys:
                ...
                if val is None:
                    raise ValueError(f"Missing key action.{key} in policy response")
                val_t = val[t] if (hasattr(val, "shape") and len(val.shape) > 0) else val
                val_t = np.array(val_t, dtype=np.float32).reshape(-1)
                action_components.extend(val_t.tolist())
            actions.append(np.array(action_components, dtype=np.float32))
        return np.stack(actions, axis=0)  # (infer_chunk, 7)
...
class Args:
    ...
    resize_size: int = 256
    infer_chunk: int = 10
    save_videos_root: str = "/tmp/mimicgen_abs_eval_results"
...
def eval_abs_mimicgen(args: Args) -> None:
    ...
    try:
        mypolicy = Gr00tn15AbsInference(args.pretrained_model_path, args.infer_chunk)
        logging.info(f"Loaded abs policy from {args.pretrained_model_path}")
    except Exception as e:
    ...
    for task_name in tasks_to_run:
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task), desc=task_name):
            ...
            # Start in delta mode so zero dummy actions keep the robot still during wait.
            for robot_obj in env.env.robots:
            ...
            while t < max_steps + args.num_steps_wait:
                ...
                # Switch to absolute mode after wait phase.
                if t == args.num_steps_wait:
                    for robot_obj in env.env.robots:
                        robot_obj.controller.use_delta = False

                action_chunk = mypolicy.get_action(obs, task_instruction)
