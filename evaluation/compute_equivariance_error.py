...
def rotate_normalized_state(state, angle_rad: float):
    ...
    Position (x, y): irrep(1).  Orientation rule depends on active data config:
      • EquiRelLibero (axis-angle [3:6]) — rotation-matrix composition.
      • EquiLibero    (quaternion [3:7]) — q_new = q_z(θ) · q_old.
    """
...
def action_to_comparison_form(action_real: np.ndarray) -> np.ndarray:
    ...
    Both target the same 10-dim comparison space so L2 is faithful and
    free of axis-angle / quaternion sign ambiguity.
    """
    ...
    for sample_idx in range(M):
        ...
        if args.bypass_normalization:
            ...
            for r in range(1, args.n_group):
                ...
                # See same helper on the equillm branch for layout.  Reads:
                #   state_diff  > 0  ⇒  state rotation is reaching the model
                #   noise_diff  > 0  ⇒  noise rotation is being applied
