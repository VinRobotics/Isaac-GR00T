...
class Args:
    ...
    n_group: int = 8
    """|G| — group order for C_N used to DISCRETIZE the test rotations (paper uses C_8).

    This is independent of the checkpoint's own ARCHITECTURAL cyclic group order
    (backbone_cfg["n_group"] / action_head_cfg["n_group"], baked in at fine-tune
    time via the data config's get_rotation_config() — see
    gr00t/model/action_head/flow_matching_action_head.py and
    gr00t/model/backbone/eagle_backbone_fa_tokens.py). A model trained with C4
    is only exactly equivariant to rotations that are multiples of 90°; testing
    it here with n_group=16 (22.5° steps) measures how it degrades OFF those
    angles, not a fair like-for-like number. When comparing a C4 checkpoint
    against a C16 checkpoint, set --args.n_group to match each checkpoint's own
    trained group order (4 and 16 respectively) and compare the two runs'
    epsilon_eq, rather than running both checkpoints at a single shared n_group."""
    ...
    for sample_idx in range(M):
        ...
        if args.bypass_normalization:
            ...
            for r in range(1, args.n_group):
                ...
                # Verifies each stage of the pipeline:
                #   (a) state: norm(state_rotated - state_orig) — should be > 0
                #   (b) noise: norm(init_noise_r - init_noise_0) — > 0 iff
                #             ρ_action(g_r) actually moves the noise (only on
                #             equi branches; baseline gets None → NaN)
                #   (c) model out: norm(a_rot - a_orig) — does the model
                #             respond at all to (image+state+noise) rotation?
                #   (d) equi gap: norm(a_rot - rotate(a_orig)) in axis-angle
                #             space (pre-6D), what the script used to report
                #   (e) baseline: norm(a_orig - rotate(a_orig)) — pure
                #             rotation of the original action (model-indep)
                #
                # Read:
                #   • If (c) ≈ 0 and (e) ≈ (d) → model output is INVARIANT to
                #     input rotation (real bug — model doesn't respond)
                #   • If (c) ≫ 0 and (d) ≈ 0 → model IS equivariant
                #   • If (c) ≫ 0 but (d) ≫ 0 → model responds but not
                #     equivariantly
