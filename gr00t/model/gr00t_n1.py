...
class GR00T_N1_5(PreTrainedModel):
    ...
    def __init__(
        ...
        # Sync n_group from backbone → action_head so old checkpoints (which only
        # persisted n_group in backbone_cfg) still reconstruct correctly.
        backbone_n_group = config.backbone_cfg.get("n_group", 4)
        if config.action_head_cfg.get("n_group", 8) != backbone_n_group:
            print(f"Syncing action_head n_group: {config.action_head_cfg.get('n_group', 8)} → {backbone_n_group}")
    ...
    def get_action_with_noise(
        ...
        and shape contract. Intended for compute_equivariance_error.py, not
