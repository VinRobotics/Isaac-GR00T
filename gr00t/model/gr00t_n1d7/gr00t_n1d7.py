...
class Gr00tN1d7ActionHead(nn.Module):
    ...
    def get_action_with_features(
        ...
        if "action" in action_input:
            ...
            actions[:, : options["rtc_overlap_steps"], : self.action_dim] = action_input["action"][
                :,
...
class Gr00tN1d7(PreTrainedModel):
    ...
    def __init__(
        ...
        # Effective motion-query-token count: None means "use the full flat
        # point set" (see Gr00tN1d7Config.num_motion_tokens), 0 when the head
        # is disabled — mirrors the old keypoint head's
        # `config.keypoint_n_key or n_total` resolution.
        num_motion_tokens = (
            (
                config.num_motion_tokens
                or config.max_motion_objects * config.motion_points_per_object
            )
        ...
        # VLM-backbone motion-keypoint head (see gr00t/model/gr00t_n1d7/motion_head.py) —
        # reads the backbone's motion_query_tokens hidden states, entirely
        # independent of the action head/DiT.
    ...
    def forward(self, inputs: dict) -> BatchFeature:
        ...
        if self.motion_head is not None:
            ...
            if motion_token_features is not None:
                motion_outputs = self.motion_head(motion_token_features, action_inputs)
