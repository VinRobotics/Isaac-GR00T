...
class EagleBackboneFA(nn.Module):
    ...
    def forward_eagle(self, vl_input: BatchFeature) -> tuple:
        ...
        _, pool_vision_features = self.eagle_model.extract_feature(rotated_pixel_values) # B * num_imgs * N
        pool_vision_features = pool_vision_features.reshape(B * self.num_images_per_sample, self.n_group, -1)
        avg_vision_features = self._apply_frame_averaging(pool_vision_features, B * self.num_images_per_sample)  # [B*num_imgs, D]
        avg_vision_features = avg_vision_features.reshape(B, self.num_images_per_sample, -1) # [B, num_imgs, D]
        ...
        language_features = language_features.hidden_states[self.select_layer]  # [B*N, T, 2048]
    ...
    def forward(self, vl_input: BatchFeature) -> BatchFeature:
        ...
        return BatchFeature(
            data={
                "backbone_vision_features": vision_embs, # Batch, Num_Imgs, 1152
                "backbone_language_features": language_embs, # Batch, token, 2048 
