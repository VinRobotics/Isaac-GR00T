...
class FlowmatchingActionHead(nn.Module):
    ...
    def __init__(
        ...
        print("Using torque aware", self.config.max_action_dim if not self.torque_aware else self.config.max_action_dim + self.effort_dim)
    
    def getJointFieldType(self, is_action):
        max_dim = self.config.max_action_dim + (self.effort_dim if self.torque_aware else 0) if is_action else self.config.max_state_dim
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        action_dim = self.action_type.size
        pred_only_actions = pred_actions[..., :action_dim-self.effort_dim]
        velocity_actions = velocity[..., :action_dim-self.effort_dim]
        # pred_actions = self.getActionOutput(pred_actions)
        # Slice out only the action portion of pred and target.
        with torch.no_grad():
            B, T, original_action_dim = action_input.action_mask.shape
            velocity_dim = velocity_actions.shape[-1]
