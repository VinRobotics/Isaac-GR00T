...
class AGITwotHand2CamConfig(BaseDataConfig):
    # video_keys = ["video.cam_left", "video.cam_right"]
    video_keys = ["video.cam_front", "video.cam_overview"]
    # video_keys = ["video.cam_front"]
    state_keys = [
...
class VRH3TwotHand1CamEquiConfig(BaseDataConfig):
    ...
    def modality_config(self):
        ...
        state_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.ee_state_keys + self.state_keys,
        )
        action_modality = ModalityConfig(
            delta_indices=self.action_indices,
            modality_keys=self.ee_action_keys + self.action_keys,
        )
        language_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.language_keys,
    ...
    def transform(self):
        transforms = [
            ...
            ConcatTransform(
                video_concat_order=self.video_keys,
                state_concat_order=self.ee_state_keys + self.state_keys,
                action_concat_order=self.ee_action_keys + self.action_keys,
            ),
            GR00TTransform(
                state_horizon=len(self.observation_indices),
                action_horizon=len(self.action_indices),
...
class EquiRelMimicgenConfig(BaseDataConfig):
    video_keys = ["video.image", "video.wrist_image"]
    # video.image (index 0) = top/head camera → equivariant FA
    # video.wrist_image (index 1) = wrist camera → VL only, skipped from equi_vision_embs
    state_keys = [
...
class EquiALOHA_2Hand_Config(BaseDataConfig):
...
class EquiRelRobocasaConfig(BaseDataConfig):
    ...
    def get_rotation_config(cls) -> dict:
        ...
        return {
            "num_images_per_sample": 3,
            "rotate_image_indices": [0, 1],  # Both right and left cameras are rotated for FA
        }
...
class EquiRelRobocasa_1cam_2_Config(BaseDataConfig):
...
DATA_CONFIG_MAP = {
    ...
    "agi_two_hand_2_cam": AGITwotHand2CamConfig(),
    "libero": LiberoConfig(),
    ...
    "equi_rel_robocasa": EquiRelRobocasaConfig(),
    "equi_rel_robocasa_1cam": EquiRelRobocasa_1cam_Config(),
    "equi_rel_robocasa_1cam_2": EquiRelRobocasa_1cam_2_Config(),
}
