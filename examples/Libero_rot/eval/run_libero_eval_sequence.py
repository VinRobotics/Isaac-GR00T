...
from examples.Libero_rot.eval.utils import (
...
class GenerateConfig:
    ...
    diverse_init: bool = False                       # Use pre-baked rotated states for diverse initialization
    rotated_states_dir: str = ""# Number of steps in action chunk returned by GR00T policy (should be >= max demo length in task suite)
    #################################################################################################################
    # fmt: on
    """Port to connect to."""
    port: int = 5555
...
class GR00TPolicy:
    ...
    def __init__(self, host="localhost", port=5555, headless=False, action_horizon=16):
    ...
    def get_action(self, observation_dict, lang: str):
        ...
        Fetches a new action chunk (horizon=16) from the policy only when the
        previous chunk has been fully executed, then replays actions one-by-one.
        """
        if self.action_chunk is None or self.current_idx >= self.action_horizon:
            obs_dict = self._process_observation(observation_dict, lang)
            self.action_chunk = self.policy.get_action(obs_dict)
            self.current_idx = 0
        action = self._convert_to_libero_action(self.action_chunk, self.current_idx)
        self.current_idx += 1
        return action
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        # Load pre-baked rotated states if diverse_init is requested
        if cfg.diverse_init:
            rotated_states, _ = load_rotated_states(
                cfg.task_suite_name,
                task_id,
                rotated_states_dir=cfg.rotated_states_dir or None,
            )
        gr00t_policy = GR00TPolicy(host="localhost", port=cfg.port, headless=cfg.headless, action_horizon=cfg.action_horizon)
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            env.reset()
            if cfg.diverse_init:
                # Use a pre-baked rotated state — no runtime physics settling needed
