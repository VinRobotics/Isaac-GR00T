...
def get_absolute_action(env, action):
    """
    Computes absolute end-effector action from relative action.
    
    The absolute action consists of:
    - goal_pos: target end-effector position (3D)
    - goal_ori: target end-effector orientation as axis-angle (3D)
    - gripper_action: gripper control value (1D)
    
    Args:
        env: The environment with robot controller
        action: The original relative action
        
    Returns:
        Absolute action as concatenated [pos (3D), axis_angle (3D), gripper (1D)]
    """
    goal_pos = env.env.robots[0].controller.goal_pos
    goal_ori_mat = env.env.robots[0].controller.goal_ori
    goal_ori_quat = T.mat2quat(goal_ori_mat)
    goal_ori_axisangle = T.quat2axisangle(goal_ori_quat)
    gripper_action = action[-1:]
    
    abs_action = np.concatenate([goal_pos, goal_ori_axisangle, gripper_action])
    return abs_action
...
def main(args):
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for demo_key in sorted(orig_data.keys(), key=lambda k: int(k.split("_")[1])):
            # Get demo data
            ...
            for _, action in enumerate(orig_actions):
                ...
                if eef_quat[3] < 0:  # enforce w >= 0 for consistent short-path axis-angle
                    eef_quat = -eef_quat
                ee_states.append(
                    np.hstack(
                        (
                            obs["robot0_eef_pos"],
                            T.quat2axisangle(eef_quat),
                        )
                    )
                )
                ...
                # Compute absolute action after step
                abs_action = get_absolute_action(env, action)
            ...
            if done:
                ...
                ep_data_grp.create_dataset("actions", data=np.stack(abs_actions, axis=0))
